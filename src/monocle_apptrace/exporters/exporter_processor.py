...
class ExportTaskProcessor(ABC):
    ...
    def queue_task(
        self,
        async_task: Callable[[Callable, any], any] = None,
        args: any = None,
...
class LambdaExportTaskProcessor(ExportTaskProcessor):
    ...
    def __init__(
        self, span_check_interval_seconds: int = 1, max_time_allowed_seconds: int = 30
    ):
    ...
    def set_sagemaker_model(self, endpoint_name: str, span: dict[str, dict[str, str]]):
        try:
            ...
            model_name_id = ""
            try:
                model_name_id = model_name_response["PrimaryContainer"]["Environment"][
                    "HF_MODEL_ID"
                ]
            except:
                pass
            ...
            logger.error(
                f"LambdaExportTaskProcessor| Failed to get sagemaker model. {e}"
            )
                ...
                for span in export_args["batch"]:
                    try:
                        if len(span["attributes"]["sagemaker_endpoint_name"]) > 0:
                            self.set_sagemaker_model(
                                endpoint_name=span["attributes"][
    ...
    def _start_async_processor(self):
        ...
        def process_tasks():
            while True:
                ...
                while (
                    root_span_found is False
                    and total_time_elapsed < self.max_time_allowed
                ):
                    logger.debug(response.json())
                    # Get next task from internal queue
                    logger.info(
                        f"[{LAMBDA_EXTENSION_NAME}] Async thread running, waiting for task from handler"
                    )
                    while self.async_tasks_queue.empty() is False:
                        logger.info(
                            f"[{LAMBDA_EXTENSION_NAME}] Processing task from handler"
                        )
                        ...
                        if async_task is None:
                            ...
                            logger.debug(
                                f"[{LAMBDA_EXTENSION_NAME}] Received null task. Ignoring."
                            )
                        else:
                            # Invoke task
                            logger.debug(
                                f"[{LAMBDA_EXTENSION_NAME}] Received async task from handler. Starting task."
                            )
                            async_task(arg)
                    total_time_elapsed += self.span_check_interval
                    logger.info(
                        f"[{LAMBDA_EXTENSION_NAME}] Waiting for root span. total_time_elapsed: {total_time_elapsed}, root_span_found: {root_span_found}."
                    )
                    time.sleep(self.span_check_interval)

                logger.debug(
                    f"[{LAMBDA_EXTENSION_NAME}] Finished processing task. total_time_elapsed: {total_time_elapsed}, root_span_found: {root_span_found}."
                )

        # Start processing extension events in a separate thread
        threading.Thread(
            target=process_tasks, daemon=True, name=LAMBDA_EXTENSION_NAME
        ).start()
