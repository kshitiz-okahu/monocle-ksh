...
class SpanExporterBase(ABC):
    def __init__(self, export_monocle_only: bool = True):
        ...
        self.export_monocle_only = export_monocle_only or os.environ.get(
            "MONOCLE_EXPORTS_ONLY", True
        )
                ...
                while attempt < retries:
                    try:
                        ...
                        sleep_time = min(
                            ...
                            backoff_in_seconds * (2 ** (attempt - 1)),
                        )
                        sleep_time = sleep_time * (
                            1 + random.uniform(-0.1, 0.1)
                        )  # Add jitter
                        logger.warning(
                            f"Network connectivity error, Attempt {attempt} failed: {e}. Retrying in {sleep_time:.2f} seconds..."
                        )
