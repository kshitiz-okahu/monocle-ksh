...
from monocle_apptrace.exporters.exporter_processor import (
    LambdaExportTaskProcessor,
...
monocle_exporters: Dict[str, Any] = {
    "s3": {
        "module": "monocle_apptrace.exporters.aws.s3_exporter",
        "class": "S3SpanExporter",
    },
    "blob": {
        "module": "monocle_apptrace.exporters.azure.blob_exporter",
        "class": "AzureBlobSpanExporter",
    },
    "okahu": {
        "module": "monocle_apptrace.exporters.okahu.okahu_exporter",
        "class": "OkahuSpanExporter",
    },
    "file": {
        "module": "monocle_apptrace.exporters.file_exporter",
        "class": "FileSpanExporter",
    },
    "memory": {
        "module": "opentelemetry.sdk.trace.export.in_memory_span_exporter",
...
def get_monocle_exporter(exporters_list: str = None) -> List[SpanExporter]:
    ...
    task_processor = (
        LambdaExportTaskProcessor() if is_aws_lambda_environment() else None
    )
    ...
    for exporter_name in exporter_names:
        ...
        try:
            ...
            warnings.warn(
                f"Unsupported Monocle span exporter '{exporter_name}', skipping."
            )
            continue
        try:
            ...
            if task_processor is not None and exporter_module.__name__.startswith(
                "monocle_apptrace"
            ):
    ...
    if not exporters:
        logger.debug(
            "No valid Monocle span exporters configured. Defaulting to FileSpanExporter."
        )
