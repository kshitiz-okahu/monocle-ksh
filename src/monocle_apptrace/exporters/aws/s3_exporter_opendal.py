...
class OpenDALS3Exporter(SpanExporterBase):
    ...
    def __upload_to_s3(self, span_data_batch: str, is_root_span: bool = False):
        ...
        try:
            ...
            logger.info(
                f"Span batch uploaded to S3 as {file_name}. Is root span: {is_root_span}"
            )
            ...
            if "404" in str(e):
                logger.error(
                    "Bucket does not exist. Please check the bucket name and region."
                )
                raise Exception(f"Bucket does not exist. Error: {e}")
            else:
                logger.error(
                    f"Unexpected error when accessing bucket {self.bucket_name}: {e}"
                )
                raise e
