...
class OpenDALAzureExporter(SpanExporterBase):
    def __init__(
        ...
        self.last_export_time = (
            time.time()
        )  # Add this line to initialize last_export_time
        ...
        endpoint, account_name, account_key = self.parse_connection_string(
            connection_string
        )

        if not account_name or not account_key:
            raise ValueError(
                "AccountName or AccountKey missing in the connection string."
            )
    ...
    def parse_connection_string(self, connection_string):
        ...
        if not all([account_name, account_key, endpoint_suffix]):
            raise ValueError(
                "Invalid connection string. Ensure it contains AccountName, AccountKey, and EndpointSuffix."
            )
    ...
    def __upload_to_opendal(self, span_data_batch: str, is_root_span: bool = False):
        ...
        try:
            ...
            logger.error(
                f"Access to container {self.container_name} is forbidden (403)."
            )
            raise PermissionError(
                f"Access to container {self.container_name} is forbidden."
            )
            ...
            if "404" in str(e):
                logger.error(
                    "Container does not exist. Please check the container name."
                )
                raise Exception(f"Container does not exist. Error: {e}")
            else:
                logger.error(
                    f"Unexpected NotFound error when accessing container {self.container_name}: {e}"
                )
                raise e
