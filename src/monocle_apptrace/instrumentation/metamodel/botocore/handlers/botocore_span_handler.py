...
class BotoCoreSpanHandler(SpanHandler):
    ...
    def _botocore_processor(
        self, to_wrap, wrapped, instance, args, kwargs, return_value
    ):
        ...
        if service_name in service_method_mapping:
            ...
            if original_method:
                ...
                if instrumentor:
                    instrumented_method = instrumentor(
                        to_wrap, wrapped, span_name, return_value, original_method
                    )
                    setattr(return_value, method_name, instrumented_method)

    def post_tracing(self, to_wrap, wrapped, instance, args, kwargs, return_value):
        self._botocore_processor(
            to_wrap=to_wrap,
            wrapped=wrapped,
            instance=instance,
