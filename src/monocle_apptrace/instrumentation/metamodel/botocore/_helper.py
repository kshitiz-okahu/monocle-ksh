    ...
    try:
        ...
        if args and isinstance(args, dict) and len(args) > 0:
            ...
            if "messages" in args and isinstance(args["messages"], list):
                ...
                user_message = extract_query_from_content(
...
def get_exception_status_code(arguments):
    if (
        arguments["exception"] is not None
        and hasattr(arguments["exception"], "response")
        and arguments["exception"].response is not None
    ):
        if (
            "ResponseMetadata" in arguments["exception"].response
            and "HTTPStatusCode" in arguments["exception"].response["ResponseMetadata"]
        ):
...
def extract_assistant_message(arguments):
    try:
        ...
        role = "assistant"
        if status == "success":
            if "Body" in arguments["result"] and hasattr(
                arguments["result"]["Body"], "_raw_stream"
            ):
...
def update_span_from_llm_response(response, instance):
    ...
    if response is not None and isinstance(response, dict) and "usage" in response:
        ...
        if token_usage is not None:
            ...
            meta_dict.update(
                {
                    "completion_tokens": resolve_from_alias(
                        token_usage,
            ...
            meta_dict.update(
                {
                    "prompt_tokens": resolve_from_alias(
                        token_usage, ["prompt_tokens", "input_tokens", "inputTokens"]
                    )
                }
            )
            meta_dict.update(
                {
                    "total_tokens": resolve_from_alias(
