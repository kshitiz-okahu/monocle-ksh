...
from monocle_apptrace.instrumentation.metamodel.finish_types import (
    map_langchain_finish_reason_to_finish_type,
)
        ...
        if args and isinstance(args, (list, tuple)) and len(args) > 0:
            if isinstance(args[0], list) and len(args[0]) > 0:
                ...
                if (
                    ...
                    and hasattr(first_msg, "type")
                    and first_msg.type == "human"
                ):
...
def extract_assistant_message(arguments):
    ...
    if status == "success":
        if isinstance(arguments["result"], str):
            ...
            hasattr(arguments["result"], "content")
            and arguments["result"].content != ""
        ):
            role = (
                arguments["result"].type
                if hasattr(arguments["result"], "type")
                else role
            )
            messages.append({role: arguments["result"].content})
        elif (
            hasattr(arguments["result"], "message")
            and hasattr(arguments["result"].message, "content")
            and arguments["result"].message.content != ""
        ):
            role = (
                arguments["result"].type
                if hasattr(arguments["result"], "type")
...
def extract_provider_name(instance):
    ...
    if hasattr(instance, "client"):
        provider_url: Option[str] = try_option(
            getattr, instance.client, "universe_domain"
        )
    if (
        hasattr(instance, "client")
        and hasattr(instance.client, "_client")
        and hasattr(instance.client._client, "base_url")
    ):
        # If the client has a base_url, extract the host from it
        provider_url: Option[str] = try_option(
            getattr, instance.client._client.base_url, "host"
        )
...
def extract_inference_endpoint(instance):
    ...
    if hasattr(instance, "client") and hasattr(instance.client, "transport"):
        inference_endpoint: Option[str] = try_option(
            getattr, instance.client.transport, "host"
        )

    if (
        hasattr(instance, "client")
        and hasattr(instance.client, "meta")
        and hasattr(instance.client.meta, "endpoint_url")
    ):
        inference_endpoint: Option[str] = try_option(
    ...
    if hasattr(instance, "client") and hasattr(instance.client, "_client"):
        inference_endpoint: Option[str] = try_option(
            getattr, instance.client._client, "base_url"
        ).map(str)
