...
def extract_messages(arguments):
    ...
    try:
        ...
        try:
            system_prompt = (
                kwargs.get("template").prompt.sections[0].sections[0].template
            )
    ...
    try:
        ...
        if hasattr(prompt, "prompt") and prompt.prompt is not None:
            if "_text" in prompt.prompt.__dict__:
                ...
                "_sections" in prompt.prompt.__dict__
                and prompt.prompt._sections is not None
            ):
...
def get_prompt_template(arguments):
    ...
    return {
        ...
        "prompt_template_description": get_nested_value(
            arguments.get("kwargs", {}), ["prompt", "config", "description"]
        ),
        "prompt_template_type": get_nested_value(
            arguments.get("kwargs", {}), ["prompt", "config", "type"]
        ),
    }
