...
from monocle_apptrace.instrumentation.metamodel.finish_types import (
    map_llamaindex_finish_reason_to_finish_type,
)
...
def get_tool_description(arguments):
    if len(arguments["args"]) > 1:
        if hasattr(arguments["args"][1], "metadata") and hasattr(
            arguments["args"][1].metadata, "description"
        ):
            return arguments["args"][1].metadata.description
        return ""
    else:
        if hasattr(arguments["instance"], "metadata") and hasattr(
            arguments["instance"].metadata, "description"
        ):
...
def get_source_agent(parent_span: Span) -> str:
    ...
    if source_agent_name == "" and parent_span.name.startswith(
        ...
        def process_message(msg):
            ...
            if hasattr(msg, "content") and hasattr(msg, "role"):
                ...
                content = (
                    msg.content
                    if role == "system"
                    else extract_query_from_content(msg.content)
                )
...
def extract_assistant_message(arguments):
    ...
    if status == "success":
        ...
        if hasattr(arguments["result"], "message") and hasattr(
            arguments["result"].message, "content"
        ):
            role = getattr(arguments["result"].message, "role", role)
            if hasattr(role, "value"):
                role = role.value
            messages.append({role: arguments["result"].message.content})
        if hasattr(arguments["result"], "response") and isinstance(
            arguments["result"].response, str
        ):
...
def extract_query_from_content(content):
    try:
        ...
        logger.warning(
            "Warning: Error occurred in extract_query_from_content: %s", str(e)
        )
        return ""
...
def extract_provider_name(instance):
    if hasattr(instance, "api_base"):
        provider_url: Option[str] = try_option(getattr, instance, "api_base").and_then(
            lambda url: urlparse(url).hostname
        )
    if hasattr(instance, "_client"):
        provider_url: Option[str] = try_option(
            getattr, instance._client.base_url, "host"
        )
    if (
        hasattr(instance, "model")
        and isinstance(instance.model, str)
        and "gemini" in instance.model.lower()
    ):
...
def extract_inference_endpoint(instance):
    if hasattr(instance, "_client"):
        if hasattr(instance._client, "sdk_configuration"):
            inference_endpoint: Option[str] = try_option(
                getattr, instance._client.sdk_configuration, "server_url"
            ).map(str)
    ...
    if (
        ...
        inference_endpoint = try_option(
            lambda: f"https://generativelanguage.googleapis.com/v1beta/models/{instance.model}:generateContent"
        )
...
def __get_host_from_map(my_map, keys_to_check):
    for key in keys_to_check:
        seed_connections = get_nested_value(
            my_map, [key, "transport", "seed_connections"]
        )
    ...
    if response is not None and hasattr(response, "raw"):
        if response.raw is not None:
            token_usage = (
                ...
                else getattr(response.raw, "usage", None)
            )
            if token_usage is None:
                token_usage = (
                    response.raw.get("usage_metadata")
                    if isinstance(response.raw, dict)
                    else getattr(response.raw, "usage_metadata", None)
                )
            if token_usage is not None:
                ...
                meta_dict.update(
                    {
                        ...
                        or token_usage.get("candidates_token_count", None)
                    }
                )
                meta_dict.update(
                    {
                        ...
                        or token_usage.get("prompt_token_count", None)
                    }
                )
                ...
                if total_tokens is not None:
                    ...
                    if output_tokens is not None and input_tokens is not None:
                        ...
                        meta_dict.update(
                            {"total_tokens": token_usage.get("total_token_count", None)}
                        )
