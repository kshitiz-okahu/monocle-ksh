...
def extract_question_from_prompt(content):
    try:
        ...
        logger.warning(
            "Warning: Error occurred in extract_question_from_prompt: %s", str(e)
        )
        return ""
...
def extract_assistant_message(arguments):
    ...
    if status == "success":
        ...
        if "replies" in arguments["result"]:
            ...
            if (
                hasattr(reply, role)
                and hasattr(reply, role, "value")
                and isinstance(reply.role.value, str)
            ):
...
def extract_inference_endpoint(instance):
    if (
        hasattr(instance, "_model_name")
        and isinstance(instance._model_name, str)
        and "gemini" in instance._model_name.lower()
    ):
        inference_endpoint = try_option(
            lambda: f"https://generativelanguage.googleapis.com/v1beta/models/{instance._model_name}:generateContent"
        )
    if hasattr(instance, "client") and hasattr(instance.client, "base_url"):
        inference_endpoint: Option[str] = try_option(
            getattr, instance.client, "base_url"
        ).map(str)
...
def extract_embeding_model(instance):
    ...
    return pipeline.map(
        lambda p: try_option(getattr, p, "get_component")
        .map(lambda g: try_option(getattr, g("text_embedder"), "model").unwrap_or(None))
        .unwrap_or(None)
    ).unwrap_or(None)
