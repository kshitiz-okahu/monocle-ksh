...
def extract_assistant_message(arguments):
    try:
        ...
        role = "assistant"
        if (
            hasattr(arguments["result"], "candidates")
            and len(arguments["result"].candidates) > 0
            and hasattr(arguments["result"].candidates[0], "content")
            and hasattr(arguments["result"].candidates[0].content, "role")
        ):
...
def update_output_span_events(results):
    if (
        hasattr(results, "embeddings")
        and isinstance(results.embeddings, list)
        and len(results.embeddings) > 0
    ):
        embeddings = results.embeddings[0]
        if (
            hasattr(embeddings, "values")
            and isinstance(embeddings.values, list)
            and len(embeddings.values) > 100
        ):
...
def extract_inference_endpoint(instance):
    try:
        if hasattr(instance, "_api_client") and hasattr(
            instance._api_client, "_http_options"
        ):
