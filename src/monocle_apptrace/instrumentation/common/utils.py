    ...
    def _with_tracer(tracer, handler, to_wrap):
        def wrapper(wrapped, instance, args, kwargs, source_path=None):
            try:
                ...
                if _parent_span_context is not None and _parent_span_context.get(
                    _SPAN_KEY, None
                ):
    ...
    return tuple(
        next(
            (
                ...
                if key.endswith(k) and value is not None
            ),
            None,
        )
        for k in keys
...
def load_scopes() -> dict:
    ...
    scope_methods = []
    try:
        scope_config_file_path = os.environ.get(
            SCOPE_CONFIG_PATH, os.path.join(os.getcwd(), SCOPE_METHOD_FILE)
        )
...
def set_scopes(scopes: dict[str, object], baggage_context: Context = None) -> object:
    ...
    for scope_name, scope_value in scopes.items():
        ...
        baggage_context = baggage.set_baggage(
            f"{MONOCLE_SCOPE_NAME_PREFIX}{scope_name}", scope_value, baggage_context
        )
...
def get_baggage_for_scopes():
    ...
    for scope_key, scope_value in get_scopes():
        ...
        baggage_context = baggage.set_baggage(
            monocle_scope_name, scope_value, context=baggage_context
        )
...
def get_exception_status_code(arguments):
    if (
        ...
        and hasattr(arguments["exception"], "code")
        and arguments["exception"].code is not None
    ):
...
def patch_instance_method(obj, method_name, func):
    ...
    new_cls = type(f"Patched{cls.__name__}", (cls,), {method_name: func})
