    ...
    def _instrument(self, **kwargs):
        ...
        for method_config in final_method_list:
            ...
            try:
                ...
                if not handler:
                    logger.warning(
                        "incorrect or empty handler falling back to default handler"
                    )
                ...
                logger.error(
                    f"""_instrument wrap exception: {str(ex)}
    ...
    def _uninstrument(self, **kwargs):
        for wrapped_method in self.instrumented_method_list:
            try:
                ...
                logger.error(
                    f"""_instrument unwrap exception: {str(ex)}
...
def setup_monocle_telemetry(
    ...
    """
    resource = Resource(attributes={SERVICE_NAME: workflow_name})
    if span_processors and monocle_exporters_list:
        raise ValueError(
            "span_processors and monocle_exporters_list can't be used together"
        )
    exporters: List[SpanExporter] = get_monocle_exporter(monocle_exporters_list)
    span_processors = span_processors or [
        BatchSpanProcessor(exporter) for exporter in exporters
    ]
    ...
    instrumentor = MonocleInstrumentor(
        user_wrapper_methods=wrapper_methods or [],
        exporters=exporters,
...
def on_processor_start(span: Span, parent_context):
    ...
    if context_properties is not None:
        for key, value in context_properties.items():
            span.set_attribute(f"{SESSION_PROPERTIES_KEY}.{key}", value)
