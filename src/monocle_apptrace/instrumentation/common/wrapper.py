...
def get_auto_close_span(to_wrap, kwargs):
    try:
        if to_wrap.get("output_processor") and to_wrap.get("output_processor").get(
            "is_auto_close"
        ):
    ...
    if not (SpanHandler.is_root_span(span) or get_value(ADD_NEW_WORKFLOW) == True):
        ...
        try:
            handler.post_task_processing(
                to_wrap, wrapped, instance, args, kwargs, return_value, span
            )
        except Exception as e:
...
def monocle_wrapper_span_processor(
    tracer: Tracer,
    handler: SpanHandler,
    to_wrap,
    wrapped,
    ...
    with tracer.start_as_current_span(name, end_on_exit=auto_close_span) as span:
        ...
        if SpanHandler.is_root_span(span) or add_workflow_span:
            ...
            return_value, span_status = monocle_wrapper_span_processor(
                tracer,
                handler,
                to_wrap,
                wrapped,
            ...
            try:
                ...
                if (
                    ex is None
                    and not auto_close_span
                    and to_wrap.get("output_processor")
                    and to_wrap.get("output_processor").get("response_processor")
                ):
                    to_wrap.get("output_processor").get("response_processor")(
                        to_wrap, return_value, post_process_span_internal
                    )
                else:
    ...
    try:
        ...
        try:
            handler.post_tracing(
                ...
                kwargs,
                return_value,
                token=pre_trace_token,
            )
        except Exception as e:
            logger.info(f"Warning: Error occurred in post_tracing: {e}")


async def amonocle_wrapper_span_processor(
    tracer: Tracer,
    handler: SpanHandler,
    to_wrap,
    wrapped,
    ...
    with tracer.start_as_current_span(name, end_on_exit=auto_close_span) as span:
        ...
        if SpanHandler.is_root_span(span) or add_workflow_span:
            # Recursive call for the actual span
            return_value, span_status = await amonocle_wrapper_span_processor(
                tracer,
                handler,
                to_wrap,
                wrapped,
...
async def amonocle_wrapper(
    ...
    try:
        ...
        try:
            handler.post_tracing(
                to_wrap, wrapped, instance, args, kwargs, return_value, pre_trace_token
            )
        except Exception as e:
            logger.info(f"Warning: Error occurred in post_tracing: {e}")
...
def evaluate_scope_values(args, kwargs, to_wrap, scope_values):
    if callable(scope_values):
        try:
            ...
            logger.warning(
                "Warning: Error occurred in evaluate_scope_values: %s", str(e)
            )
            scope_values = None
