...
class SpanHandler:
    ...
    def post_tracing(
        self, to_wrap, wrapped, instance, args, kwargs, return_value, token=None
    ):
        pass
    ...
    def set_span_type(
        self, to_wrap, wrapped, instance, output_processor, span: Span, args, kwargs
    ) -> str:
        span_type: str = None
    ...
    def post_task_processing(
        self, to_wrap, wrapped, instance, args, kwargs, result, span: Span
    ):
        pass
    ...
    def hydrate_span(
        ...
        try:
            detected_error_in_attribute = self.hydrate_attributes(
                to_wrap, wrapped, instance, args, kwargs, result, span, parent_span
            )
            detected_error_in_event = self.hydrate_events(
                to_wrap, wrapped, instance, args, kwargs, result, span, parent_span, ex
            )
    ...
    def hydrate_attributes(
        ...
        if SpanHandler.is_root_span(span):
            span_index = (
                2  # root span will have workflow and hosting entities pre-populated
            )
        if "output_processor" in to_wrap and to_wrap["output_processor"] is not None:
            output_processor = to_wrap["output_processor"]
            self.set_span_type(
                to_wrap, wrapped, instance, output_processor, span, args, kwargs
            )
            skip_processors: list[str] = (
                self.skip_processor(to_wrap, wrapped, instance, span, args, kwargs)
                or []
            )
                    ...
                    for processor in processors:
                        ...
                        if attribute and accessor:
                            ...
                            try:
                                ...
                                if processor_result and isinstance(
                                    processor_result, (str, list)
                                ):
                            ...
                            logger.debug(
                                f"{' and '.join([key for key in ['attribute', 'accessor'] if not processor.get(key)])} not found or incorrect in entity JSON"
                            )
                    span_index += 1
    ...
    def hydrate_events(
        ...
        if "output_processor" in to_wrap and to_wrap["output_processor"] is not None:
            ...
            if "events" in output_processor and (
                "events" not in skip_processors or ex is not None
            ):
                    ...
                    for attribute in attributes:
                        ...
                        if accessor:
                            try:
                                ...
                                logger.debug(
                                    f"Error evaluating accessor for attribute '{attribute_key}': {e}"
                                )
                    matching_timestamp = getattr(ret_result, "timestamps", {}).get(
                        event_name, None
                    )
    ...
    def set_app_hosting_identifier_attribute(span):
        ...
        for type_env, type_name in service_type_map.items():
            if type_env in os.environ:
                span.set_attribute(
                    f"entity.{span_index}.type", f"app_hosting.{type_name}"
                )
                entity_name_env = service_name_map.get(type_name, "unknown")
                span.set_attribute(
                    f"entity.{span_index}.name",
                    os.environ.get(entity_name_env, "generic"),
                )

    @staticmethod
    def get_workflow_name(span: Span) -> str:
        try:
            return get_value("workflow_name") or span.resource.attributes.get(
                "service.name"
            )
        except Exception as e:
    ...
    def is_root_span(curr_span: Span) -> bool:
        try:
            if (
                curr_span is not None
                and hasattr(curr_span, "parent")
                or curr_span.context.trace_state
            ):
        ...
        if to_wrap:
            ...
            if workflow_type != WORKFLOW_TYPE_GENERIC:
                token = attach(
                    set_value(
                        WORKFLOW_TYPE_KEY,
                        SpanHandler.get_workflow_type(to_wrap),
                        context,
                    )
                )
        return token
